...
const store = {
    ...
    dispatch (action) {
        this._dataState.profilePage = profileReduser(this._dataState.profilePage, action);
        this._dataState.dialogsPage = dialogsReduser(this._dataState.dialogsPage, action);
        this._rerenderApplication(this);
    },
};
...
//         path: path.resolve(__dirname, "./dist") // Папка в которой будет наш bundle.js собранный одни файл со всех модулей не забыть переменную __dirname
//         //Но для того чтобы webpack нашел эту папку нужен отдельный плагин path с методор resolve("./dist") только тогда он будет искать этот файл везде
//     },
